...
export default function App() {
  ...
  const DEFAULT_PRODUCTS: Product[] = [
    { id: '1', code: 'NASSER-101', name: 'ماكينة إعداد القهوة الإسبيرسو الاحترافية NASSER Pro 3', category: 'أجهزة ومعدات', stock: 45, minStock: 5, unit: 'جهاز', updatedAt: new Date().toISOString() },
    { id: '2', code: 'NASSER-102', name: 'طاحونة حبوب القهوة الصناعية 1500W دقيقة التنعيم', category: 'أجهزة ومعدات', stock: 22, minStock: 3, unit: 'قطعة', updatedAt: new Date().toISOString() },
    { id: '3', code: 'NASSER-103', name: 'طابعة فواتير حرارية عالية السرعة 80mm USB/LAN', category: 'إلكترونيات ومعدات', stock: 18, minStock: 4, unit: 'طابعة', updatedAt: new Date().toISOString() },
    { id: '4', code: 'NASSER-104', name: 'ميزان إلكتروني ديجيتال دقيق للوزن والجرعات 0.1g', category: 'أجهزة قياس', stock: 4, minStock: 5, unit: 'ميزان', updatedAt: new Date().toISOString() },
    { id: '5', code: 'NASSER-105', name: 'فلتر تنقية وتقطير المياه خماسي المراحل للمقاهي', category: 'مستلزمات ومستهلكات', stock: 60, minStock: 10, unit: 'طقم', updatedAt: new Date().toISOString() },
    { id: '6', code: 'NASSER-106', name: 'مقبض ضغط القهوة اليدوي (Tamper) استانلس ستيل 58mm', category: 'ملحقات ومستلزمات', stock: 85, minStock: 15, unit: 'قطعة', updatedAt: new Date().toISOString() }
  ];
  ...
  // Batch Add Products
  const handleBatchAddProducts = async (items: Array<{ code?: string; name: string; stock: number; category?: string; minStock?: number; unit?: string; description?: string }>) => {
    try {
